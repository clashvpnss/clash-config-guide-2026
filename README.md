...
> 📄 **想直接用现成的？** → [`config.yaml`](config.yaml)：可直接使用的完整配置模板，
> 换上你的订阅链接即可。已包含正确的规则顺序、DoH 防污染、AI 服务固定出口分组。

| 你的问题 | 跳转 |
| --- | --- |
| 有没有能直接抄的完整配置？ | [config.yaml 模板](config.yaml) |
...
上面讲的所有要点，都落在仓库里的 [`config.yaml`](config.yaml) 中，可直接使用：

- **43 条规则**，顺序已排好——`GEOIP,CN` 在第 42 条、`MATCH` 在最后一条
- **8 个分组**，其中 `🤖 AI 与账号` 用 `fallback` 固定出口，避免 IP 跳变触发风控
- **DNS 用 DoH + `fallback-filter`** 防污染，同时避免国内域名被解析到境外
- `fake-ip-filter` 已排除游戏 NAT 穿透、NTP 对时、Windows 连通性检测等需真实 IP 的场景
- 每段都有注释说明「为什么这么写」

用法：把 `proxy-providers` 里的订阅链接换成你自己的，其余可不动。

> ⚠️ 适用 **Mihomo（Clash Meta）内核**。原版 Clash 已停止维护，部分字段在其上无效。
